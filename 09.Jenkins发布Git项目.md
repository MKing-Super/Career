...
EUREKA_IP_PORT="10.50.10.113:9100"
...
ssh root@${TARGET_IP} "mkdir -p ${JAR_FILE_PATH}; mkdir -p ${JAR_RILE_LOG_PATH}; " && \
# 文件传输
scp $JAR_FILE root@${TARGET_IP}:${JAR_FILE_PATH}  && \

echo "连接服务器 ${TARGET_IP}， 停止正在运行的实例并启动jar"
ssh root@${TARGET_IP} << EOF
  APP_PID=$(ps -ef | grep '${MODULE_NAME}' | grep -v grep | awk '{print $2}')
  if [ -n "$APP_PID" ]; then
    echo "Stopping existing ${MODULE_NAME} application with PID $APP_PID"
    kill $APP_PID
    sleep 5
  fi

  nohup /usr/local/java/bin/java -jar ${JAR_FILE_PATH}/$FILE_NAME \
  		--eureka.client.service-url.defaultZone=http://${EUREKA_IP_PORT}/eureka/ >> "$LOG_FILE" 2>&1 &
 
  # 参数配置
  MAX_RETRY=30      # 最大重试次数
  INTERVAL=5        # 每次检测间隔（秒）
  TIMEOUT=2         # curl超时时间（秒）

  # 健康检查循环
  for i in $(seq 1 $MAX_RETRY); do
  	echo "curl -sSf --max-time $TIMEOUT '$APP_URL'"
    if curl -sSf --max-time $TIMEOUT "$APP_URL" | grep -q '"status":"UP"'; then
  ...
  echo "${TARGET_IP} Application started. Logs: tail -f $LOG_FILE"
EOF
```
