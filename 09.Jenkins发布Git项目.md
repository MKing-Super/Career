...
/data/data/com.termux/files/home/projects/start
...
# 项目 绝对路径
PROJECT_ABS_PATH="/data/data/com.termux/files/home/storage/shared/MkProjects/Career/Projects/springcloud-demo"
# Jar包、日志存放 绝对路径
...
echo "${MODULE_FOLDER_NAME} Maven install 开始 >>>>>>>>>>>>"
cd ${PROJECT_ABS_PATH}/${MODULE_FOLDER_NAME} && \
mvn clean install -N  && \
echo "${MODULE_FOLDER_NAME} Maven install 结束 <<<<<<<<<<<<<<"
```

jenkins

```shell
START_SH_NAME="springcloud_demo_api.sh"
PROJECT_RUN_PATH="/data/data/com.termux/files/home/projects"

bash "${PROJECT_RUN_PATH}/start/${START_SH_NAME}"
```
...
MODULE_NAME="eureka-server"
# 模块文件夹名称
MODULE_FOLDER_NAME="eureka-server"
# eureka注册中心IP、端口
EUREKA_IP="127.0.0.1"
EUREKA_PORT="9100"
# 项目运行的服务器IP、端口
...
echo "${MODULE_FOLDER_NAME} Maven打包 开始 >>>>>>>>>>>>"
cd ${PROJECT_ABS_PATH}/${MODULE_FOLDER_NAME} && \
mvn clean package && \
echo "${MODULE_FOLDER_NAME} Maven打包 结束 <<<<<<<<<<<<<<"

mkdir -p ${JAR_FILE_PATH}
mkdir -p ${JAR_FILE_LOG_PATH}
JAR_TARGET_FOLDER_PATH="${PROJECT_ABS_PATH}/${MODULE_FOLDER_NAME}/target" && \
JAR_ABS_PATH=$(ls $JAR_TARGET_FOLDER_PATH/*.jar) && \
JAR_RUN_PATH="${JAR_FILE_PATH}/${MODULE_NAME}.jar" && \
echo "打包路径：$JAR_ABS_PATH"
echo "运行路径：${JAR_RUN_PATH}"
mv $JAR_ABS_PATH ${JAR_RUN_PATH} && \

APP_PID=$(ps -ef | grep "${MODULE_NAME}" | grep -v grep | awk '{print $2}')
if [ -n "$APP_PID" ]; then
  echo "停止正在运行的实例 PID $APP_PID"
  kill $APP_PID
  # 立即检查进程状态
  if kill -0 $APP_PID 2>/dev/null; then
    echo "进程仍在运行 (kill是异步的!)"
  else
    echo "进程已结束"
  fi
  # 等待进程结束
...
nohup "${JAVA_PATH}" -jar ${JAR_RUN_PATH} \
--server.port="${TARGET_PORT}" \
--spring.application.name="${MODULE_NAME}" \
>> $LOG_FILE 2>&1 &
...
```shell
START_SH_NAME="eureka_server.sh"
PROJECT_RUN_PATH="/data/data/com.termux/files/home/projects"

bash "${PROJECT_RUN_PATH}/start/${START_SH_NAME}" 1
```
...
MODULE_NAME="api-gateway"
# 模块文件夹名称
MODULE_FOLDER_NAME="api-gateway"
# eureka注册中心IP、端口
EUREKA_IP="127.0.0.1"
EUREKA_PORT="9100"
# 项目运行的服务器IP、端口
...
MODULE_NAME="home-service"
# 模块文件夹名称
MODULE_FOLDER_NAME="home-service"
# eureka注册中心IP、端口
EUREKA_IP="127.0.0.1"
EUREKA_PORT="9100"
# 项目运行的服务器IP、端口
...
# 模块名称
MODULE_NAME="netty-demo"
# 模块文件夹名称
MODULE_FOLDER_NAME="netty-demo"
# 项目运行的服务器IP、端口
TARGET_IP="127.0.0.1"
WEBSOCKET_SERVER_PORT="8860"
NETTY_SERVER_PORT="8861"

# Java 绝对路径
JAVA_PATH=$(which java)
# 项目 绝对路径
PROJECT_ABS_PATH="/data/data/com.termux/files/home/storage/shared/MkProjects/Career/Projects"
# Jar包、日志存放 绝对路径
PROJECT_RUN_PATH="/data/data/com.termux/files/home/projects"
JAR_FILE_PATH="${PROJECT_RUN_PATH}/jars"
JAR_FILE_LOG_PATH="${PROJECT_RUN_PATH}/logs"
...
nohup "${JAVA_PATH}" -jar ${JAR_RUN_PATH} ${WEBSOCKET_SERVER_PORT} ${NETTY_SERVER_PORT} \
>> $LOG_FILE 2>&1 &
