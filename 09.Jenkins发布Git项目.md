...
ssh root@${TARGET_IP} << EOF
  ...
  echo "nohup /usr/local/java/bin/java -jar ${JAR_FILE_PATH}/$FILE_NAME --eureka.client.service-url.defaultZone=http://10.50.10.113:9100/eureka/ > ${JAR_RILE_LOG_PATH}/${MODULE_NAME}.log 2>&1 "
  
  nohup /usr/local/java/bin/java -jar ${JAR_FILE_PATH}/$FILE_NAME --eureka.client.service-url.defaultZone=http://10.50.10.113:9100/eureka/ > ${JAR_RILE_LOG_PATH}/${MODULE_NAME}.log 2>&1 &
	echo "Application started. Logs: tail -f ${JAR_RILE_LOG_PATH}/${MODULE_NAME}.log"
EOF
```
